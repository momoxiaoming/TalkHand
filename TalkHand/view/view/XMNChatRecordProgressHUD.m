...
@interface XMNChatRecordProgressHUD ()
...
@implementation XMNChatRecordProgressHUD
...
+ (XMNChatRecordProgressHUD *)sharedView {
    static dispatch_once_t once;
    static XMNChatRecordProgressHUD *sharedView;
    dispatch_once(&once, ^ {
        sharedView = [[XMNChatRecordProgressHUD alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
...
+ (void)show {
    [[XMNChatRecordProgressHUD sharedView] show];
}

+ (void)dismissWithProgressState:(XMProgressState)progressState{
    [[XMNChatRecordProgressHUD sharedView] setProgressState:progressState];
    [[XMNChatRecordProgressHUD sharedView] dismiss];
}

+ (void)dismissWithMessage:(NSString *)message{
    [[XMNChatRecordProgressHUD sharedView] setProgressState:XMProgressMessage];
    [XMNChatRecordProgressHUD sharedView].centerLabel.text = message;
    [[XMNChatRecordProgressHUD sharedView] dismiss];
}

+ (void)changeSubTitle:(NSString *)str
{
    [[XMNChatRecordProgressHUD sharedView] setSubTitle:str];
}

+ (NSTimeInterval)seconds{
    return [[XMNChatRecordProgressHUD sharedView] seconds] / 10;
}
